...
class UserController extends Controller {
    ...
    public function accessRules() {
        return array(
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('index', 'view', 'savetrack', 'tracklist', 'logout', 'isprojectexisted', 'getfunchinform', 'getfunchinformsubmit'),
                'users' => array('*'),
            ),
    ...
    public function actionIsProjectExisted() {
        if ($_POST) {
            ...
            $trackRecordObject = TrackRecord::model()->findByAttributes(array('user_id' => $userId, 'project_id' => $projectId, 'created_at' => $projectDate, 'from_time' => ''));
            if (!empty($trackRecordObject)) {
                echo CJSON::encode(array('to_time' => $trackRecordObject->to_time, 'description' => $trackRecordObject->description));
                exit;
            } else {
                echo "0";
            }
     ...
     * @param type $divId
     */
    public function actiongetFunchInForm() {
        $userId = Yii::app()->session['userid'];
        $todayDate = date('Y/m/d');
        $userHasPunchInObject = UserHasPunchin::model()->findAllByAttributes(
                array('status' => 1, 'uid' => $userId, 'created_at' => $todayDate));
        

        $data = $this->renderPartial('punchInOutForm', array('punchInObject' => $userHasPunchInObject));
        echo $data;
    }
     ...
     * @param type $divId
     */
    public function actiongetFunchInFormSubmit() {
        $error = NULL;
        $success = NULL;
        if (isset($_POST) && !empty($_POST)) {
            // Get logged in UID;
            $userId = Yii::app()->session['userid'];
            $model = new UserHasPunchin;

            $postDataArray = $_POST;
            $postDataArray['uid'] = $userId;

            /* Save RgmHasServices */
            $userHasPuchInObject = UserHasPunchin::model()->recordSave($postDataArray, $model);

            if (!empty($userHasPuchInObject)) {
                Yii::app()->user->setFlash('success', "Submitted Successfully.");
            }
        } else {
            Yii::app()->user->setFlash('error', "Failed : Something went wrong.");
        }

        $urlPath = Yii::app()->urlManager->createUrl('user/');
